...
\begin{figure}[tb]
\centering
\includegraphics[width=0.80\linewidth]{figures/evolution_bars.pdf}
\vspace{-2mm}
...
\vspace{-20mm}
\captionof{table}{Component measurements across the evolution rounds.}
\label{tab:subdim}
\centering
\small
...
\centering
\includegraphics[width=\linewidth]{figures/single_vs_multi.pdf}
\caption{OpenAgency versus its single-agent ablation under one model and tool budget.}
\label{fig:svm}
\end{minipage}\hfill
\begin{minipage}[t]{0.465\textwidth}
\captionof{table}{OpenAgency against the free-form and SOP baselines.}
\label{tab:free_form}
\centering
\small
...
\caption{Evolution on OfficeQA Pro from one initialization, scored on the same $133$ tasks. The previous-protocol arm (MetaGPT) edits only the functional division.}
\label{fig:fixed}
\end{minipage}\hfill
\begin{minipage}[t]{0.44\textwidth}
\captionof{table}{Governance activity on the three deployed $v_4$ protocols that carry a handoff chain, as the share of proposed handoffs $\Gamma(n,s)$ rules out and the share of runs whose outcome that refusal changes.}
\label{tab:handoff}
\centering
\small
...
\end{minipage}
\end{figure}
To measure what a fixed collaboration graph costs, we plot the OfficeQA Pro evolution record as two arms. The previous-protocol arm applies MetaGPT~\citep{hong2024metagptmetaprogrammingmultiagent}. Figure~\ref{fig:fixed} shows six such rewrites peaking at $31.6$ in round $2$, the four later ones falling back below it. The full arm, which may also change the agent count, reaches $62.4$ with one added reader node and $69.9$ after four rounds, matching Table~\ref{tab:evolution_all}. A fixed graph keeps the search inside a level the trace already reports as saturated, and one structural edit clears that ceiling by more than thirty points.

\section{Conclusion}

We introduced OpenAgency, a self-evolving multi-agent system whose governed collaboration protocol makes the collaboration pattern an evolvable object, updated from ordered traces under a validity screen. It leads the compared systems on all fourteen benchmarks.
