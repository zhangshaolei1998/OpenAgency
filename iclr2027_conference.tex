...
\vspace{-5mm}
\caption{Three nested evolving spaces under one initialization and one evolution loop. The fixed-count arm holds the number of agents $|\mathcal{N}|$ at the initialization, and the division-only arm freezes the whole graph and edits only the functional division.}
\label{fig:evo_space}
\vspace{-3mm}
\end{wrapfigure}
To identify which levels of the evolving space carry the gain, we restrict that space in two nested steps and rerun the same evolution loop on three benchmarks. The full arm edits all three levels of Section~\ref{sec:self_evolving}. Holding the number of agents $|\mathcal{N}|$ at the initialization gives the fixed-count arm, which still edits the handoff structure $(\mathcal{H},\mu)$ and the functional division. Freezing the whole graph gives the division-only arm, where the optimizer reaches the role assignment $\alpha$, the completion predicates $\Omega$, and the shared $\mathcal{C}$, the space a hand-designed multi-agent system still leaves open after deployment. Figure~\ref{fig:evo_space} reports the best protocol each arm reaches, and the three benchmarks order the arms identically. Freezing the agent count alone costs $5.8$ points on SWE-bench Verified, $12.6$ on DevEval, and $15.3$ on DDR-Bench, so the freedom to add an agent is the level that pays most. Freezing the remaining handoff structure costs a further $2.3$ to $5.2$ points. A hand-designed system commits both structural levels before any trace has been read, and holding all three inside one governed protocol defers that choice to the trace.
