...
\vspace{-3mm}
\end{wrapfigure}
To identify which levels of the evolving space carry the gain, we restrict that space in two nested steps and rerun the same evolution loop on three benchmarks. The full arm edits all three levels of Section~\ref{sec:self_evolving}. Holding the number of agents $|\mathcal{N}|$ at the initialization gives the fixed-count arm, which still edits the handoff structure $(\mathcal{H},\mu)$ and the functional division. Freezing the whole graph gives the division-only arm, where the optimizer reaches the role assignment $\alpha$, the completion predicates $\Omega$, and the shared $\mathcal{C}$. Figure~\ref{fig:evo_space} reports the best protocol each arm reaches, and the three benchmarks order the arms identically. Freezing the agent count alone costs $5.8$ points on SWE-bench Verified, $12.6$ on DevEval, and $15.3$ on DDR-Bench, so the freedom to change the agent count is the level that pays most. Freezing the remaining handoff structure costs a further $2.3$ to $5.2$ points. A hand-designed system commits both structural levels before any trace has been read, while one governed protocol defers that choice to the trace.

\subsection{Comparison against Free-form and SOP Baselines}
\label{sec:free_form}

To compare the three collaboration substrates directly, we evaluate OpenAgency against the free-form baseline AutoGen \citep{wu2023autogenenablingnextgenllm} and the SOP baselines ChatDev \citep{qian2024chatdevcommunicativeagentssoftware} and MetaGPT \citep{hong2024metagptmetaprogrammingmultiagent} on four benchmarks. OpenAgency obtains the highest score in Table~\ref{tab:free_form}, exceeding the strongest SOP baseline by $26.4$ points on SWE-bench Verified. The free-form baseline is the weakest everywhere, and the SOP baselines recover part of the gap because the collaboration order is enforced. Their fixed procedure absorbs no trace-level failure signal, so the remaining gap tracks what stays revisable after deployment.
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.76\linewidth]{figures/fixed_topology.pdf}
\vspace{-2mm}
...
\label{fig:fixed}
\end{figure}
To measure what a fixed collaboration graph costs round by round, we plot the OfficeQA Pro evolution record as two arms from one initialization, where the fixed arm reproduces how a standard multi-agent system tunes a prompt over a hand-designed structure. Every plotted point is a candidate the optimizer proposed and executed.

Figure~\ref{fig:fixed} shows six functional-division rewrites peaking at $31.6$ in round $2$, with the four later rewrites all falling back below it. Widening the graph moves it at once, since one added reader node reaches $62.4$ and one added verifier node reaches $66.2$. The full arm has a lower round too, where a functional-division candidate falls to $60.9$ and the screen rejects it. A fixed graph keeps the search inside a level the trace already reports as saturated, while one admissible structural edit clears the fixed ceiling by more than thirty points.
