...
\label{fig:evo_space}
\end{wrapfigure}
To identify which levels of the evolving space carry the gain, we restrict that space in two nested steps and rerun the same evolution loop on three benchmarks. The full arm edits all three levels of Section~\ref{sec:self_evolving}, the fixed-count arm holds the agent count $|\mathcal{N}|$, and the division-only arm freezes the whole graph. Figure~\ref{fig:evo_space} reports the best protocol each arm reaches, and the three benchmarks order the arms identically. Freezing the agent count alone costs $5.8$ points on SWE-bench Verified, $12.6$ on DevEval, and $15.3$ on DDR-Bench, so the freedom to change the agent count is the level that pays most, and freezing the remaining handoff structure costs a further $2.3$ to $5.2$ points. In essence, a hand-designed system commits both structural levels before reading any trace.
...
To measure what a fixed collaboration graph costs, we plot the OfficeQA Pro evolution record as two arms from one initialization. The previous-protocol arm applies MetaGPT~\citep{hong2024metagptmetaprogrammingmultiagent} and rewrites only the functional division over a single-node graph. As shown in Figure~\ref{fig:fixed}, six such rewrites peak at $31.6$ in round $2$, and the four later ones fall back below it. The full arm, which may also change the agent count and the handoff structure, reaches $62.4$ once a reader node is added and $69.9$ after four rounds, matching Table~\ref{tab:evolution_all}. Both arms share the executor, the tool budget, and the $133$ scored tasks, so the gap comes from the level an edit may reach. A fixed graph keeps the search inside a level the trace already reports as saturated, and one structural edit clears that ceiling by more than thirty points.
