...
\end{gather}
where $\hat{\mathcal{P}}^{(v+1)}$ denotes the candidate obtained by applying $\Delta^{(v)}$ to $\mathcal{P}^{(v)}$, and the hat marks a candidate not yet screened.
...
\begin{gather}
    ...
    \end{cases}
\end{gather}
so a candidate is deployed only when it is well formed and does not degrade the observed coordination.
