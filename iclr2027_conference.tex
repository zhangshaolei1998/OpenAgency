...
\While{active nodes exist}
    \For{each active node $n$ with role $r = \alpha(n)$}
        ...
        \ElsIf{$e$ is a finish $(n, y)$ and $\Omega_n(y)$ holds}
            \State mark $n$ complete, activate every committed target whose started predecessors have handed off
        \Else
...
\textbf{Admissibility and Quality Screen.}\quad A step is screened twice before it is deployed. The first check is a structural validity operator $\Pi_{\mathcal{V}}$ on the candidate topology, which rejects malformed handoff edges, unresolved role references, duplicate node identifiers, and completion states no path can reach, so a candidate that is not a well-formed protocol never runs. Writing $\tilde{\mathcal{P}}^{(v+1)} = \Pi_{\mathcal{V}}\bigl(\hat{\mathcal{P}}^{(v+1)}\bigr)$ for the validity-checked candidate, the second check executes it and hands the resulting traces to the evolving agent. The agent returns a collaboration quality score $q_{\mathrm{col}}$ from the realized coordination, reading whether each role delivered what its successors required and whether the completion conditions closed on a usable output. The deployment rule is
\begin{gather}
    \mathcal{P}^{(v+1)} =
    \begin{cases}
...
We evaluate the governed protocol of Section~\ref{sec:method} on fourteen benchmarks (Section~\ref{sec:benchmarks}) under one executor and tool budget (Section~\ref{sec:setup}), and report the main comparison in Section~\ref{sec:main_results}. Section~\ref{sec:analysis} then locates the gain, through protocol evolution round by round (Section~\ref{sec:evolution_effect}), its component measurements (Section~\ref{sec:subdim}), the levels of the evolving space (Section~\ref{sec:ablation_space}), multi-agent protocols (Section~\ref{sec:ablation_multi}), free-form and SOP substrates (Section~\ref{sec:free_form}), and a fixed collaboration graph (Section~\ref{sec:fixed_topology}). Appendix~\ref{sec:admissible} reports the activity of the governance boundary.
...
To separate governed collaboration from the executor's own capability, we use GPT-5.5 across all benchmarks, and each role receives the tool surface its benchmark requires. For each benchmark, we compare three in-house conditions under one model and tool budget, a single-agent ablation with no handoff, the unstructured multi-agent initialization $v_0$, and the evolved protocol after four rounds ($v_4$). All three are reported against a prior reference (Prior Ref.) that fixes the absolute level of the benchmark, taken as the strongest publicly reported result we found for each benchmark at the time of submission, and as a same-model direct-LLM baseline where no stronger external result is available. Each benchmark evolves its own protocol independently, and the evolving agent reads only execution traces, so no task label or reference answer reaches it, following the label-free self-refinement setting of \citet{shinn2023reflexion} and \citet{madaan2023selfrefine}. Model, temperature, and tool budget stay fixed across rounds, and only the protocol template changes. Table~\ref{tab:main} reports the final round and Appendix~\ref{app:detail} lists all four.
...
% \end{wrapfigure}
\begin{table}[t]
\caption{OpenAgency and its ablations against a selected prior reference that anchors the absolute level of each benchmark. Multi-agent~(init) is the unstructured initialization and OpenAgency is the evolved protocol, both sharing the GPT-5.5 executor and tool budget of the single-agent ablation. Each row uses the benchmark's native metric, where EM is exact match, FM is file match, and AT is the acceptance-test pass rate.}
\label{tab:main}
\begin{center}
\small
...
HLE (text-only) & Accuracy & --- & 18.4 & 5.6 & \textbf{37.7} \\
\midrule
...
\textbf{Capability on General Question Answering.}\quad The five single-turn benchmarks (HumanEval, MBPP, GSM8K, MATH, HotpotQA) probe \emph{short-chain reasoning capabilities}, since each instance has a localized, automatically verifiable output and leaves little room to coordinate. Against its two ablations and each benchmark's prior reference, OpenAgency obtains the best primary metric on all five in Table~\ref{tab:main}, reaching $100.0$ pass@1 on HumanEval and lifting MBPP from $73.5$ to $80.9$ over the initialization. Agent count alone does not produce this gain, since the unstructured initialization trails the single-agent ablation on HotpotQA at $64.0$ against $75.9$ F1 while the governed protocol carries the same benchmark to $82.3$. Evolution prunes handoffs whose upstream output the trace flags as ineffective, and each benchmark settles on a solve-critique-revise loop matched to its dominant failure mode, so the protocol turns a short chain into a verified one.


\textbf{Capability on Long-horizon Tasks.}\quad SWE-bench Verified, DeepPlanning, and HLE stress \emph{long-horizon reasoning capabilities}, since each instance requires several dependent decisions and a bad handoff at any stage can invalidate the entire run. Against the single-agent ablation and each benchmark's prior reference, Table~\ref{tab:main} reports a lead on all three, reaching $86.2$ file match on SWE-bench Verified against a prior reference of $42.8$ and $37.7$ accuracy on HLE. The protocol each benchmark converges to shows what produces the lead. On SWE-bench Verified the evolving agent installs a retry edge when a patch fails and splits an overloaded Verifier into structural and semantic components. The DeepPlanning protocol hands a Verifier the user-specified constraints one at a time, turning a brittle end-to-end plan into locally verifiable handoffs. Verification on HLE becomes conditional, routing high-confidence answers to completion and low-confidence ones back through a retry edge. All three shapes come from the same initialization, so the handoff and completion rules adapt when local errors are unrecoverable.




\textbf{Capability on Production-oriented Tasks.}\quad The six production-oriented benchmarks (DevEval, DDR-Bench, TheAgentCompany, MARBLE, AutomationBench, OfficeQA Pro) test \emph{cross-domain deployment capabilities}, since each exposes a different tool surface and task format. OpenAgency leads on all six in Table~\ref{tab:main}, improving over the prior reference by $34.4$ points on DevEval, $19.6$ points on MARBLE, and $12.8$ points on OfficeQA Pro. The template differs per benchmark, from a Reader-Solver-Verifier chain on OfficeQA Pro to a Generator-Critic-Reviser pipeline on MARBLE, and two cases expose what governance contributes. On DDR-Bench, an Auditor role introduced during evolution forces every quantitative finding in the trace into the final report, fixing a silent omission that free-form baselines leave undetected. The admissible-set constraint on MARBLE keeps each handoff terminating under a protocol that would otherwise allow any-to-any messaging, so in both cases governance supplies a constraint the task format alone does not.
...
\caption{Round-wise trajectory of the primary metric from the initialization $v_0$ through the evolved rounds $v_1$--$v_4$. Grey dashed lines mark each benchmark's prior reference.}
\label{fig:evolution}
\end{figure}
...
To verify that bounded edits keep improving collaboration after the first revision, we track the primary metric from the initialization $v_0$ to the evolved rounds $v_1$ through $v_4$ on all fourteen benchmarks. Figure~\ref{fig:evolution} and Table~\ref{tab:evolution_all} record an improvement over the initialization on every benchmark, reaching $48.9$ points on MARBLE and $48.8$ points on OfficeQA Pro. The deployed sequence is non-decreasing on every benchmark in collaboration quality, since the screen keeps the incumbent whenever a candidate scores lower. On the fourteen benchmarks studied, this ordering also holds for the primary task metric.


\subsection{Component Measurements behind the Gain}
\label{sec:subdim}

To locate the gain inside a benchmark, we decompose the SWE-bench Verified and DevEval results of every round into their component measurements. Table~\ref{tab:subdim} shows the two improving at different points. On SWE-bench Verified the file-match rate given a patch moves only from $84.6$ to $86.2$, while the fraction of instances producing a patch at all rises from $79.2$ to $100.0$, so the gain is consistent with closing the no-output failure mode that the retry edges of Section~\ref{sec:self_evolving} target. On DevEval the unit-test rate stays near saturation while acceptance rises from $82.1$ to $96.9$, so the admitted edits are associated with the repository-level correctness that unit tests leave unchecked. In both cases the component that moves aligns with the edit the evolving agent targeted.

\begin{table}[t]
\caption{Component measurements across the evolution rounds. Patch applied is the fraction of instances producing a non-empty patch, and file match given a patch restricts file match to those instances.}
\label{tab:subdim}
\begin{center}
\small
...
\vspace{-5mm}
\caption{Three nested evolving spaces under one initialization and one evolution loop. The fixed-count arm holds the number of agents $|\mathcal{N}|$, and the division-only arm freezes the whole graph and edits only the functional division.}
\label{fig:evo_space}
\vspace{-3mm}
\end{wrapfigure}
To identify which levels of the evolving space carry the gain, we restrict that space in two nested steps and rerun the same evolution loop on three benchmarks. The full arm edits all three levels of Section~\ref{sec:self_evolving}. Holding the number of agents $|\mathcal{N}|$ at the initialization gives the fixed-count arm, which still edits the handoff structure $(\mathcal{H},\mu)$ and the functional division, and freezing the whole graph gives the division-only arm, which reaches only the role assignment $\alpha$, the completion predicates $\Omega$, and the shared $\mathcal{C}$. Figure~\ref{fig:evo_space} reports the best protocol each arm reaches, and the three benchmarks order the arms identically. Freezing the agent count alone costs $5.8$ points on SWE-bench Verified, $12.6$ on DevEval, and $15.3$ on DDR-Bench, so on these three benchmarks the freedom to change the agent count is the level that pays most, and freezing the remaining handoff structure costs a further $2.3$ to $5.2$ points. A hand-designed system commits both structural levels before any trace has been read, while one governed protocol defers that choice to the trace.


\subsection{Advantage of Multi-agent Protocols}
\label{sec:ablation_multi}

To attribute the gain to governance and not to agent count, we compare OpenAgency with its single-agent ablation under one GPT-5.5 executor, identical tool access, and identical temperature. OpenAgency exceeds that ablation on every benchmark in Figure~\ref{fig:svm}, with the widest gaps on OfficeQA Pro, DevEval, and DDR-Bench. A single node interleaves solving and checking inside one context, while the governed protocol routes a partial output to a downstream role whose completion predicate validates it independently. The initialization column of Table~\ref{tab:main} places the gain on protocol design, since the same initial agent count without governance falls short.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.66\linewidth]{figures/fixed_topology.pdf}

\caption{Evolution on OfficeQA Pro from one initialization, scored on the same $133$ tasks. The previous-protocol arm (MetaGPT) edits only the functional division, while the full arm may also change the agent count and the topology.}
\label{fig:fixed}
\end{figure}
...
To compare the three collaboration substrates directly, we evaluate OpenAgency against the free-form baseline AutoGen \citep{wu2023autogenenablingnextgenllm} and the SOP baselines ChatDev \citep{qian2024chatdevcommunicativeagentssoftware} and MetaGPT \citep{hong2024metagptmetaprogrammingmultiagent} on four benchmarks. OpenAgency obtains the highest score in Table~\ref{tab:free_form}, exceeding the strongest SOP baseline by $26.4$ points on SWE-bench Verified. The free-form baseline is the weakest everywhere, and the SOP baselines recover part of the gap because the collaboration order is enforced, yet their fixed procedure absorbs no trace-level failure signal, so the remaining gap tracks what stays revisable after deployment.


\subsection{Superiority of Governed Protocols over Fixed Graphs}
\label{sec:fixed_topology}

To measure what a fixed collaboration graph costs round by round, we plot the OfficeQA Pro evolution record as two arms from one initialization. The previous-protocol arm applies MetaGPT~\citep{hong2024metagptmetaprogrammingmultiagent} and tunes only the functional division over that fixed structure. Figure~\ref{fig:fixed} shows six such rewrites peaking at $31.6$ in round $2$, with the four later rewrites all falling back below it. The full arm, which may also change the agent count and topology, reaches $62.4$ with one added reader node and $69.9$ after four rounds, matching Table~\ref{tab:evolution_all}. A fixed graph keeps the search inside a level the trace already reports as saturated, while one structural edit clears the previous-protocol ceiling by more than thirty points.

\section{Conclusion}

We introduced OpenAgency, a self-evolving multi-agent system built on governed collaboration protocols that turns the collaboration pattern into an evolvable object. Ordered traces become bounded, validity-screened protocol updates. OpenAgency reaches the highest primary metric among the compared systems on each of fourteen benchmarks.
...
\clearpage
\section{Activity of the Admissible Handoff Set}
...
\midrule
DDR reference agent (Prior Ref.) & 60.5 & 61.3 & 59.6 \\
...
\midrule
Direct LLM (Prior Ref.)                                              & Full PDF                & 26.3 \\
