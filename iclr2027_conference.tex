...
\label{fig:fixed}
\end{figure}
To measure what a fixed collaboration graph costs round by round, we plot the OfficeQA Pro evolution record as two arms from one initialization, where the previous-protocol arm tunes a prompt over that fixed structure. Figure~\ref{fig:fixed} shows six functional-division rewrites peaking at $31.6$ in round $2$, with the four later rewrites all falling back below it. Widening the graph moves it at once, since one added reader node reaches $62.4$ and one added verifier node reaches $66.2$. On OfficeQA Pro, a fixed graph keeps the search inside a level the trace already reports as saturated, while one structural edit clears the previous-protocol ceiling by more than thirty points.

\section{Conclusion}

We introduced OpenAgency, a self-evolving multi-agent system built on governed collaboration protocols. Constraining runtime handoffs while preserving each agent's choice among admissible collaborators turns the collaboration pattern into an evolvable object, and ordered traces become bounded, validity-screened protocol updates. OpenAgency reaches the highest primary metric among the compared systems on each of fourteen benchmarks.
