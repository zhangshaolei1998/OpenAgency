...
% \end{wrapfigure}
\begin{table}[t]
\caption{OpenAgency and its ablations against each benchmark's prior reference, a strong baseline from prior work that fixes the absolute level of the benchmark. Multi-agent~(init) is the unstructured initialization and OpenAgency is the evolved protocol, both sharing the GPT-5.5 executor and tool budget of the single-agent ablation. Each row uses the benchmark's native metric, where EM is exact match, FM is file match, and AT is the acceptance-test pass rate.}
\label{tab:main}
\begin{center}
\small
...
To locate the gain inside a benchmark, we decompose the SWE-bench Verified and DevEval results of every round into their component measurements. Table~\ref{tab:subdim} shows the two improving at different points. On SWE-bench Verified the file-match rate given a patch moves only from $84.6$ to $86.2$, while the fraction of instances producing a patch at all rises from $79.2$ to $100.0$, so the gain closes the no-output failure mode that the retry edges of Section~\ref{sec:self_evolving} target. On DevEval the unit-test rate stays near saturation while acceptance rises from $82.1$ to $96.9$, so the admitted edits act on the repository-level correctness that unit tests leave unchecked. In both cases the component that moves is the one the accepted edit was written to address.
...
\vspace{-2mm}
\caption{Evolution on OfficeQA Pro from one initialization, scored on the same $133$ tasks. The previous-protocol arm edits only the functional division, while the full arm may also change the agent count and the topology.}
\label{fig:fixed}
\end{figure}
To measure what a fixed collaboration graph costs round by round, we plot the OfficeQA Pro evolution record as two arms from one initialization, where the previous-protocol arm tunes a prompt over that fixed structure and every plotted point is a candidate the optimizer proposed and executed. Figure~\ref{fig:fixed} shows six functional-division rewrites peaking at $31.6$ in round $2$, with the four later rewrites all falling back below it. Widening the graph moves it at once, since one added reader node reaches $62.4$ and one added verifier node reaches $66.2$. The full arm has a lower round too, where a functional-division candidate falls to $60.9$ and the screen rejects it. A fixed graph keeps the search inside a level the trace already reports as saturated, while one admissible structural edit clears the previous-protocol ceiling by more than thirty points.
