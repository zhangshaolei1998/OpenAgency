...
To separate governed collaboration from the executor's own capability, we use GPT-5.5 across all benchmarks, and each role receives the tool surface its benchmark requires. For each benchmark, we compare three in-house conditions under one model and tool budget, a single-agent ablation with no handoff, the unstructured multi-agent initialization $v_0$, and the evolved protocol after four rounds ($v_4$). All three are reported against a prior system (Prior Sys.), the official GPT-5.5 baseline for each benchmark, taken as the strongest publicly reported GPT-5.5 result at the time of submission. Each benchmark evolves its own protocol independently, and the evolving agent reads only execution traces, so no task label or reference answer reaches it, following the label-free self-refinement setting of \citet{shinn2023reflexion} and \citet{madaan2023selfrefine}. Model, temperature, and tool budget stay fixed across rounds, and only the protocol template changes. Table~\ref{tab:main} reports the final round and Appendix~\ref{app:detail} lists all four.
...
% \end{wrapfigure}
\begin{table}[t]
\caption{OpenAgency and its ablations against each benchmark's official GPT-5.5 baseline (Prior Sys.). Multi-agent~(init) is the unstructured initialization and OpenAgency is the evolved protocol, both sharing the GPT-5.5 executor and tool budget of the single-agent ablation. Each row uses the benchmark's native metric, where EM is exact match, FM is file match, and AT is the acceptance-test pass rate.}
\label{tab:main}
\begin{center}
\small
...
\vspace{-4mm}
\caption{Component measurements across the evolution rounds.}
\label{tab:subdim}
\small
\centering
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.82\linewidth]{figures/fixed_topology.pdf}
\vspace{-2mm}
...
\midrule
Qwen-Agent tool-calling (Prior Sys.) & 46.9 & 88.8 & 56.7 & 72.7 & 32.9 & 86.8 & 60.8 \\
...
\begin{table}[t]
\caption{Performance on DevEval \citep{li2024deveval}. UT is the unit-test pass rate and AT is the repository-level acceptance-test pass rate. The prior system is a single coder node with no handoff, which is also the single-agent ablation.}
\label{tab:deveval}
\begin{center}
\small
\setlength{\tabcolsep}{4pt}
...
\midrule
Direct LLM (Prior Sys.)  & 90.4 & 62.5 \\
...
Claude-Sonnet-4.5 (Prior Sys.) & 69.1 & 77.6 & 60.6 \\
\midrule
...
\midrule
OpenHands (Prior Sys.) & GPT-5.5 & 40.0 & 41.5 & 31.6 \\
...
GPT-4o-mini (Prior Sys.) & GPT-4o-mini   & 84.1 & 74.5 & 79.3 \\
\midrule
...
GPT-6-Sol & 33.2 \\
GPT-6-Astra & 41.4 \\
Claude-Opus-5.5 (Prior Sys.) & 42.5 \\
\midrule
...
Claude-Opus-4.6 (Prior Sys.) & ai\_parse\_document     & 57.1 \\
\midrule
