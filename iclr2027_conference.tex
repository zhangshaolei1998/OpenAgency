...
\begin{figure}[tb]
\centering
\includegraphics[width=0.72\linewidth]{figures/fixed_topology.pdf}
\vspace{-2mm}
\caption{Evolution on OfficeQA Pro under a fixed collaboration graph against evolution over the full space, from one initialization and scored on the same $133$ tasks. The previous-protocol arm edits only the functional division, while the full arm may also change the number of agents and the topology.}
\label{fig:fixed}
\end{figure}
To measure what a fixed collaboration graph costs round by round, we plot the OfficeQA Pro evolution record as two arms from one initialization, where the previous-protocol arm reproduces how a standard multi-agent system tunes a prompt over that fixed structure. Every plotted point is a candidate the optimizer proposed and executed.

Figure~\ref{fig:fixed} shows six functional-division rewrites peaking at $31.6$ in round $2$, with the four later rewrites all falling back below it. Widening the graph moves it at once, since one added reader node reaches $62.4$ and one added verifier node reaches $66.2$. The full arm has a lower round too, where a functional-division candidate falls to $60.9$ and the screen rejects it. A fixed graph keeps the search inside a level the trace already reports as saturated, while one admissible structural edit clears the previous-protocol ceiling by more than thirty points.
