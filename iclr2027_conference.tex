...
\begin{abstract}
Large language model (LLM) agents have demonstrated strong capabilities on complex tasks, motivating the development of multi-agent systems that coordinate agents with specialized roles. However, improving how these agents collaborate requires two seemingly competing properties: flexibility, which enables diverse coordination decisions at runtime, and controllability, which allows those decisions to be inspected, attributed, and revised afterwards. Existing systems typically provide only one of these properties: predefined workflows offer explicit control but constrain execution to fixed routes, while free-form communication enables flexible interaction but leaves the realized coordination process opaque and difficult to revise. 
In this paper, we introduce \emph{OpenAgency}, a self-evolving multi-agent system built upon \emph{governed collaboration protocols}. Such protocol allows agents to select their next collaborators within the permitted space at runtime. This design turns the collaboration pattern itself into an evolvable object, which is structured enough to be analyzed yet flexible enough to adapt during execution. To evolve this protocol, OpenAgency maintains execution traces that capture not only successful interactions but also rejected handoffs, failed completions, and other coordination outcomes. A collaboration-driven evolving agent transforms this evidence into a bounded number of atomic protocol improvements. 
Experiments on fourteen benchmarks spanning general question answering, long-horizon tasks, and production-oriented workloads show that OpenAgency achieves the best primary metric among compared systems on every benchmark\footnote{Code is available in the supplementary materials.}.
\end{abstract}
...
Large language models (LLMs)~\citep{chatgpt} have demonstrated remarkable capabilities on increasingly complex tasks, including software engineering~\citep{yang2024sweagent,wang2024openhands,zhang2024autocoderover}, data research~\citep{liu2026ddrbench,li2024deveval}, and long-horizon planning~\citep{zhang2026deepplanning,xu2024theagentcompany}. Building upon these capabilities, LLM-based multi-agent systems further improve task-solving ability by assigning agents complementary roles and enabling them to exchange intermediate information through communication~\citep{wang2024survey,xi2023rise,hong2024metagptmetaprogrammingmultiagent,qian2024chatdevcommunicativeagentssoftware,wu2023autogenenablingnextgenllm}. Such collaboration allows individual agents to overcome their limitations and collectively solve tasks that are difficult for a single model. However, as tasks become longer, more diverse, and increasingly open-ended, the collaboration process itself becomes a new source of failure: ineffective coordination, unnecessary interactions, or inappropriate delegation can accumulate errors throughout execution. This raises a fundamental question: how can multi-agent collaboration itself continuously improve?

Evolving collaboration, however, is non-trivial because it requires two seemingly conflicting properties: flexibility and controllability. Flexibility enables a multi-agent system to explore diverse collaboration strategies; only by trying different coordination patterns and comparing their outcomes can the system discover better ways to cooperate. Controllability, meanwhile, is essential because an improvement signal is meaningful only when the system can identify which coordination decisions contributed to success or failure and verify that the revised collaboration remains valid. Without flexibility, evolution degenerates into selecting among a fixed set of predefined strategies; without controllability, improvement becomes blind optimization guided only by final task rewards, with little understanding of how or why a collaboration pattern changes.

Existing collaboration representations typically provide only one of these properties, limiting the possibility of continual evolution. \emph{Blueprint-driven} approaches explicitly encode collaboration structures as graphs, workflows, or standard operating procedures~\citep{hong2024metagptmetaprogrammingmultiagent,wu2023autogenenablingnextgenllm,fourney2024magenticonegeneralistmultiagentsolving}, and recent efforts further automate such designs through workflow search or generated agent programs~\citep{zhang2025aflowautomatingagenticworkflow,hu2025automateddesignagenticsystems,zhang2025maasagenticsupernet}. These approaches offer strong controllability because the coordination structure is explicitly specified. However, once a workflow is instantiated, execution largely follows a predetermined route, making it difficult to adapt the collaboration locally when unexpected situations arise. In contrast, \emph{agent-driven} approaches allow communication patterns, debates, or agent connectivity to emerge dynamically at runtime~\citep{li2023camelcommunicativeagentsmind,du2023improvingfactualityreasoninglanguage,liu2024dynamicllmpoweredagentnetwork,park2023generativeagentsinteractivesimulacra}, and learned orchestrators can further adjust agent activation orders based on feedback signals~\citep{dang2025puppeteer}. While these approaches provide flexibility, the resulting coordination process is often implicit and difficult to attribute, analyze, or systematically revise across runs. In short, existing paradigms either constrain collaboration to predefined structures or allow collaboration to emerge without sufficient transparency, leaving a gap for a representation that is both adaptable during execution and revisable through experience.

In this paper, we aim to make the collaboration pattern itself an evolvable object by achieving both flexibility and controllability. To this end, we introduce \emph{OpenAgency}, a self-evolving multi-agent system built upon \emph{governed collaboration protocols}. As illustrated in Figure~\ref{fig:intro}, OpenAgency represents collaboration as a structured protocol that specifies agent roles and the allowed ways for agents to interact. Rather than fixing a single execution path, the protocol defines a set of valid coordination choices, allowing agents to adapt their interactions at runtime while preventing invalid or undesirable behaviors. This design makes every coordination decision explicit and verifiable, enabling the collaboration pattern to be systematically improved across runs. Based on this protocol, OpenAgency collects execution traces that record coordination decisions, successful interactions, failures, and other execution outcomes. An evolving agent analyzes these traces and performs a small number of targeted edits to the collaboration protocol, allowing the system to gradually improve. Each proposed change is validated before deployment, ensuring that evolved collaboration remains grounded in previous experience.
To the best of our knowledge, OpenAgency is the first multi-agent system that treats the collaboration pattern itself as an evolvable object, allowing it to improve from immutable execution traces while preserving runtime validity and interpretability. We summarize our contributions as follows.
\begin{itemize}[leftmargin=1em, itemsep=0ex, topsep=0ex]
    ...
    \item \textbf{Collaboration-driven Evolution:} We propose an evolving agent that learns from the coordination evidence prior systems discard. A validity operator and a collaboration quality screen keep every deployed round admissible and free of regression.
    \item \textbf{Strong Performance:} Experiments across fourteen benchmarks spanning general question answering, long-horizon reasoning, and production-oriented tasks demonstrate that OpenAgency achieves the best primary metric on every benchmark, improving over unstructured multi-agent initialization by an average of $20.7$ points.
\end{itemize}
...
% \end{wrapfigure}
\begin{table}[t]
\caption{Results of OpenAgency on each benchmark. Prior Sys. is official agent with GPT-5.5 backbone. Multi-agent~(init) is the unstructured initialization and OpenAgency is the evolved protocol, both sharing the GPT-5.5 backbone. Each row uses the benchmark's native metric, where EM is exact match, FM is file match, and AT is the acceptance-test pass rate.}
\label{tab:main}
\begin{center}
\small
...
HLE (text-only) & Accuracy & 13.4 & 18.4 & 5.6 & \textbf{37.7} \\
\midrule
...
\textbf{Capability on General Question Answering.}\quad The five single-turn benchmarks (HumanEval, MBPP, GSM8K, MATH, HotpotQA) probe \emph{short-chain reasoning capabilities}, since each instance has a localized, automatically verifiable output. OpenAgency obtains the best primary metric on all five in Table~\ref{tab:main}, reaching $100.0$ pass@1 on HumanEval and lifting MBPP from $73.5$ to $80.9$ over the initialization. Agent count alone does not produce this gain, since the initialization trails the single-agent ablation on HotpotQA at $64.0$ against $75.9$ F1 while the governed protocol reaches $82.3$. Evolution prunes handoffs whose upstream output the trace flags as ineffective, and each benchmark settles on a solve-critique-revise loop matched to its dominant failure mode.

\textbf{Capability on Long-horizon Tasks.}\quad SWE-bench Verified, DeepPlanning, and HLE stress \emph{long-horizon reasoning capabilities}, since a bad handoff at any stage can invalidate the entire run. Table~\ref{tab:main} reports a lead on all three, reaching $86.2$ file match on SWE-bench Verified against a prior system of $42.8$ and $37.7$ accuracy on HLE. The protocol each benchmark converges to shows why. On SWE-bench Verified the evolving agent installs a retry edge when a patch fails and splits an overloaded Verifier into structural and semantic components. The DeepPlanning protocol hands a Verifier the constraints one at a time, turning a brittle end-to-end plan into locally verifiable handoffs. Verification on HLE becomes conditional, routing low-confidence answers back through a retry edge. All three shapes come from the same initialization, so the handoff and completion rules adapt when local errors are unrecoverable.

\textbf{Capability on Production-oriented Tasks.}\quad The six production-oriented benchmarks (DevEval, DDR-Bench, TheAgentCompany, MARBLE, AutomationBench, OfficeQA Pro) test \emph{cross-domain deployment capabilities}, since each exposes a different tool surface. OpenAgency leads on all six in Table~\ref{tab:main}, improving over the prior system by $34.4$ points on DevEval, $19.6$ points on MARBLE, and $12.8$ points on OfficeQA Pro. The template differs per benchmark, and two cases expose what governance contributes. On DDR-Bench, an Auditor role introduced during evolution forces every quantitative finding in the trace into the final report, fixing a silent omission free-form baselines leave undetected. The admissible-set constraint on MARBLE keeps each handoff terminating where any-to-any messaging would not, so governance supplies a constraint the task format alone does not.
...
To attribute the gain to governance and not to agent count, we compare OpenAgency with its single-agent ablation under one GPT-5.5 executor and identical tool access. OpenAgency exceeds that ablation on every benchmark in Figure~\ref{fig:svm}, with the widest gaps on OfficeQA Pro, DevEval, and DDR-Bench. A single node interleaves solving and checking inside one context, while the governed protocol routes a partial output to a downstream role that validates it independently. The initialization column of Table~\ref{tab:main} places the gain on protocol design, since the same agent count without governance falls short.
...
\end{tabular}
\end{wraptable}
To compare the three collaboration substrates, we evaluate OpenAgency against the free-form baseline AutoGen \citep{wu2023autogenenablingnextgenllm} and the SOP baselines ChatDev \citep{qian2024chatdevcommunicativeagentssoftware} and MetaGPT \citep{hong2024metagptmetaprogrammingmultiagent}. OpenAgency obtains the highest score in Table~\ref{tab:free_form}, exceeding the strongest SOP baseline by $26.4$ points on SWE-bench Verified. The free-form baseline is the weakest everywhere, and the SOP baselines recover part of the gap because the collaboration order is enforced, yet their fixed procedure absorbs no trace-level failure signal.
...
\vspace{-\intextsep}
\caption{Governance activity on the three deployed $v_4$ protocols that carry a handoff chain, as the share of proposed handoffs $\Gamma(n,s)$ rules out and the share of runs whose outcome that refusal changes.}
\vspace{2mm}
\label{tab:handoff}
\centering
\small
...
\end{tabular}
\end{wraptable}
To show what one governed protocol deploys once the task changes, we read out the $v_4$ protocol of each benchmark. The three protocols of Table~\ref{tab:handoff} differ in agent count, role vocabulary, and handoff order. DevEval converges on a coder checked by a reviewer, DDR-Bench on a researcher feeding a reporter, and TheAgentCompany on a planner, an executor, and a reviewer, so one loop settles on the division each task rewards. The admissible handoff set stays active on every chain, ruling out $3.8\%$ to $8.2\%$ of proposed handoffs and changing the outcome of $2.9\%$ to $5.3\%$ of runs. A fixed procedure commits to one of these patterns before any trace is seen, and the refused events are the evidence the evolving agent consumes in Section~\ref{sec:self_evolving}.
...
\begin{figure}
    \centering
\includegraphics[width=0.9\linewidth]{figures/fixed_topology.pdf}
    \vspace{-5mm}
    \caption{Evolution of OpenAgency and MetaGPT on OfficeQA Pro.}
\label{fig:fixed}
\end{figure}

To measure what a fixed collaboration graph costs, we plot the OfficeQA Pro evolution record as two arms from one initialization. The previous-protocol arm applies MetaGPT~\citep{hong2024metagptmetaprogrammingmultiagent} and rewrites only the functional division over a single-node graph. Figure~\ref{fig:fixed} shows six such rewrites peaking at $31.6$ in round $2$, with the four later ones falling back below it. The full arm, which may also change the agent count and the handoff structure, reaches $62.4$ once a reader node is added and $69.9$ after four rounds, matching Table~\ref{tab:evolution_all}. Both arms share the executor, the tool budget, and the $133$ scored tasks, so the gap comes from the level an edit may reach. A fixed graph keeps the search inside a level the trace already reports as saturated, and one structural edit clears that ceiling by more than thirty points.

\section{Conclusion}

We introduced OpenAgency, a self-evolving multi-agent system whose governed collaboration protocol makes the collaboration pattern an evolvable object, updated from ordered traces under a validity screen. Experiments across fourteen benchmarks demonstrate that OpenAgency
achieves significant and sustained improvements.
...
\begin{table}[t]
\caption{Performance on the Humanity's Last Exam text-only subset. External rows are reported by that work.}
\label{tab:hle}
\begin{center}
\small
